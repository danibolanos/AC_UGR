...
*/
 ...
 int main(int argc, char** argv){
    ...
    //Acumula el valor de la multiplicación de una fila de la matriz por el vector (v1)
    //en la variable suma_fila y luego se la asigna al vector resultado (v2)

    #pragma omp parallel for private(suma_fila, k)
       for(i=0; i<N; i++){
          suma_fila = 0;
          for(k=0; k<N; k++)
             suma_fila += M1[i][k]*v1[k];
          v2[i] = suma_fila;
       }
